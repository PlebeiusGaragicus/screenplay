---
name: 3-entity-prompts
description: Create entity prompt HTML pages from an approved chapter entity manifest. Use when the user requests Step 1.3 or entity prompt page creation.
---

# Step 1.3 — Entity prompt pages

## Goal

Create one grounded HTML prompt page per approved entity, with source quotes and links back to chapter paragraphs.
...
- Create pages only for manifest rows marked `create` unless the user approves another action.
...
Every approved `create` row in the manifest has a corresponding prompt page, and each page links back to its manifest row plus at least one source paragraph.
